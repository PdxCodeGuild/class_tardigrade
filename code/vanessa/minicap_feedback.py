...
def goal_maker(message, success_message='Yay one step closer!', failure_message='keep trying!', how_often=1):
    """factory function to create other goal functions"""
    def goal(message=message, success_message=success_message, failure_message=failure_message):
        """goal function returned by goal_maker"""
        print(message)
        counts = 0
    ...
    schedule.every(how_often).minutes.do(goal)
    return goal

goal1 = goal_maker('drink water', 'stay hydrated fam!', 'would you prefer gatorade?')
goal2 = goal_maker('work out')
goal3 = goal_maker('sleep 8 hours')
